...
void game::drawFrame(game::MapCell** map, game::GameInfo* gameInfo)		// отрисовка карты, функци€ принимает в качетсве аргумета двумерный массив структур
{
	...
	for (int i = 0; i < MAP_HEIGHT; i++)
	{
		for (int j = 0; j < MAP_WIDTH; j++)
		{
			...
			{
			case HERO:						// если встречен тип "герой", то отрисовываетс€ герой
				game::printSpecialSymbol(consoleHandler, i, j, LightGrey, HERO);
				break;

			case BLOCK:						// если встречен тип "блок", то отрисовываетс€ блок
				game::printSpecialSymbol(consoleHandler, i, j, LightGrey, BLOCK);
				break;

			case EMPTY_SPACE:				// если встречен тип "_", то отрисовываетс€ пустое пространство
				game::printSpecialSymbol(consoleHandler, i, j, Black, EMPTY_SPACE);
				break;

			case AIM_DOT:					// если встречен тип "прицел_точка", то отрисовываетс€ прицел
				game::printSpecialSymbol(consoleHandler, i, j, LightGrey, AIM);
				break;

			case BLACK_WALL:				// если встречен тип "непроходима€ стена", то отрисовываетс€ непроходима€ стена
				game::printSpecialSymbol(consoleHandler, i, j, Yellow, BLACK_WALL);
				break;

			case EXIT:					// если встречен тип "выход", то отрисовываетс€ выход
				game::printSpecialSymbol(consoleHandler, i, j, LightGrey, EXIT);
				break;

			case RED_PORTAL:			// если встречен тип "красный портал", то отрисовываетс€ красный портал
				game::printSpecialSymbol(consoleHandler, i, j, LightRed, RED_PORTAL);
				break;

			case BLUE_PORTAL:			// если встречен тип "синий портал", то отрисовываетс€ синий портал 
				game::printSpecialSymbol(consoleHandler, i, j, LightBlue, BLUE_PORTAL);
				break;

			case BUTTON:				// если встречен тип "кнопка", то отрисовываетс€ кнопка 
				game::printSpecialSymbol(consoleHandler, i, j, Yellow, BUTTON);
				break;

			case TURRET_HUNTER:				// если встречен тип "турель", то отрисовываетс€ обычна€ турель 
				game::printSpecialSymbol(consoleHandler, i, j, LightGrey, TURRET_HUNTER);
				break;

			case BULLET_OF_STATIONARY_TURRET:				// если встречен тип "пул€", то отрисовываетс€ пул€ 
				game::printSpecialSymbol(consoleHandler, i, j, LightRed, BULLET_OF_STATIONARY_TURRET);
				break;

			case BULLET_OF_PLATFORM_TURRET:				// если встречен тип "пул€", то отрисовываетс€ пул€ 
				game::printSpecialSymbol(consoleHandler, i, j, LightRed, BULLET_OF_STATIONARY_TURRET);
				break;

			case STATIONARY_TURRET:		// если встречен тип "стационарна€ турель", то отрисовываетс€ стационарна€ турель турель
				game::printSpecialSymbol(consoleHandler, i, j, LightGrey, STATIONARY_TURRET);
				break;

			case PLATFORM_TURRET:		// если встречен тип "турель на платформе", то отрисовываетс€ турель на платформе 
	...
	// ¬ывод на экран информации о текущем здоровии и времени, затраченном на прохождение уровн€
	std::cout << std::endl << "HP: " << gameInfo->hero.healthPoints << "\t\t\tTIME: " << gameInfo->hero.time;
}

// функци€ очистки экрана (позвол€ет избежать мерцани€, как в случае с system("cls"))
...
// ‘ункци€ освобождени€ пам€ти
void game::freeMemory(game::MapCell** map, game::GameInfo* gameInfo)
{
	delete gameInfo; // ќчищаем пам€ть, зан€тую информацией об игре

	// ќчищаем пам€ть, зан€тую картой игры
	for (int i = 0; i < MAP_HEIGHT; i++)
	{
		delete map[i];
	}
...
// ‘ункци€ отвечает за вывод на экран определенного символа с заданными цветом и позицией 
...
// ‘укнци€, отвечающа€ за вывод инструкции на экран
